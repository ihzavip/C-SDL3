...
/* How far the body's left edge sits from the frame's left edge, per (anim,
   dir). Non-zero when the frame is wider than the body (e.g. punch-left fist
   extends left). */
 ...
 * Direction order: DIR_DOWN=0, DIR_UP=1, DIR_LEFT=2, DIR_RIGHT=3 (from
 * entity.h)
 */
...
#define HIT_COOLDOWN_DURATION                                                  \
  1.0f /* seconds of invincibility after being hit */
#define SPRITE_HEIGHT_PADDING                                                  \
  2 /* extra px read from sheet to avoid foot clipping */
...
void player_init(SDL_Renderer *renderer) {
  ...
  for (int a = 0; a < ANIM_COUNT; a++) {
    for (int d = 0; d < DIR_COUNT; d++) {
      snprintf(path, sizeof(path),
               "media/Character/Main/%s/Character_%s_%s-%s.png", anim_dirs[a],
               dir_names[d], anim_names[a], anim_sfx[a]);
  ...
  /* Load bat character sprites — used after the bat is picked up.
     ANIM_IDLE and ANIM_RUN share the same idle-and-run sheet (loaded
     separately). ANIM_PICKUP is not used for the bat set —
     bat_textures[ANIM_PICKUP] stays NULL. */
  ...
  //     continue;
  //   for (int d = 0; d < DIR_COUNT; d++) {
  //     snprintf(path, sizeof(path), "media/Character/Bat/Bat_%s_%s.png",
  ...
  /* Load no-hands body sprites — base layer drawn beneath the bat weapon
     overlay sfx means suffix
  */
  ...
  for (int a = 0; a < ANIM_COUNT; a++) {
    if (!nohands_sfx[a])
      continue;
    float extra =
        nohands_frame_w[a][DIR_LEFT] - nohands_frame_w[ANIM_IDLE][DIR_LEFT];
  ...
  /* Hands overlay (filenames use inconsistent capitalisation) */
  const char *hands_paths[ANIM_COUNT][DIR_COUNT] = {
      /* ANIM_IDLE */
      {
          "media/Character/Main/Idle/Hands_down_idle-Sheet6.png",
          "media/Character/Main/Idle/Hands_Up_idle-Sheet6.png",
          "media/Character/Main/Idle/Hands_Side-left_idle-Sheet6.png",
          "media/Character/Main/Idle/Hands_Side_idle-Sheet6.png",
      },
      /* ANIM_RUN */
      {
          "media/Character/Main/Run/Hands_down_run-Sheet6.png",
          "media/Character/Main/Run/Hands_Up_run-Sheet6.png",
          "media/Character/Main/Run/Hands_side-left_run-Sheet6.png",
          "media/Character/Main/Run/Hands_Side_run-Sheet6.png",
      },
      /* ANIM_PUNCH */
      {
          "media/Character/Main/Punch/Hands_Down_punch-Sheet4.png",
          "media/Character/Main/Punch/Hands_Up_punch-Sheet4.png",
          "media/Character/Main/Punch/Hands_side-left_punch-Sheet4.png",
          "media/Character/Main/Punch/Hands_side_punch-Sheet4.png",
      },
      /* ANIM_PICKUP — NULL */
      {NULL, NULL, NULL, NULL},
  };
  ...
  for (int a = 0; a < ANIM_COUNT; a++) {
    for (int d = 0; d < DIR_COUNT; d++) {
      if (!hands_paths[a][d])
        continue;
      hands_textures[a][d] = load_sheet(renderer, hands_paths[a][d]);
      if (hands_textures[a][d]) {
        float w, h;
        SDL_GetTextureSize(hands_textures[a][d], &w, &h);
        hands_frame_w[a][d] = w / frame_counts[a];
        hands_frame_h[a][d] = h;
      }
    }
  }
...
void player_render(SDL_Renderer *renderer, Camera camera) {
  ...
  if (bat_equipped) {
    ...
    float body_ax = nohands_anchor_x[anim_state][dir];
    SDL_FRect body_src = {anim_frame * body_fw, 0, body_fw,
                          body_fh + SPRITE_HEIGHT_PADDING};
    SDL_FRect body_world = {player.x - body_ax, player.y, body_fw,
                            body_fh + SPRITE_HEIGHT_PADDING};
    SDL_FRect body_dst = camera_project(camera, body_world);
    if (body_tex) {
      if (blink)
        SDL_SetTextureColorMod(body_tex, 255, 80, 80);
    ...
    SDL_FRect src = {anim_frame * frame_width, 0, frame_width,
                     frame_height + SPRITE_HEIGHT_PADDING};
    // TODO: - 2
    SDL_FRect world_rect = {player.x - ax, player.y, frame_width,
                            frame_height + SPRITE_HEIGHT_PADDING};
    ...
    if (hands_tex) {
      ...
      if (blink)
        SDL_SetTextureColorMod(hands_tex, 255, 80, 80);
    ...
    // SDL_FRect physics_screen = camera_project(
    //     camera, (SDL_FRect){player.x, player.y, player.w, player.h});
...
void player_destroy(void) {
  ...
  for (int a = 0; a < ANIM_COUNT; a++)
    for (int d = 0; d < DIR_COUNT; d++)
      if (textures[a][d])
        SDL_DestroyTexture(textures[a][d]);
  for (int a = 0; a < ANIM_COUNT; a++)
    for (int d = 0; d < DIR_COUNT; d++)
      if (bat_textures[a][d])
        SDL_DestroyTexture(bat_textures[a][d]);
  for (int a = 0; a < ANIM_COUNT; a++)
    for (int d = 0; d < DIR_COUNT; d++)
      if (nohands_textures[a][d])
        SDL_DestroyTexture(nohands_textures[a][d]);
  for (int a = 0; a < ANIM_COUNT; a++)
    for (int d = 0; d < DIR_COUNT; d++)
      if (hands_textures[a][d])
        SDL_DestroyTexture(hands_textures[a][d]);
}
